...
if( !class_exists("\\" . __NAMESPACE__ . "\\PseudoCss") ) :
...
class PseudoCss
{
	...
	public function tryRequest()
	{
		...
		{
			$url = esc_url_raw($_REQUEST[self::ARG]);
			$this->toCss($url);
		}
	}
	...
	public function loadValues()
	{
		...
		{
			global $wpdb;
			$sql = $wpdb->prepare("SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE %s", $this->MasterID.'%');
			$vals = $wpdb->get_results($sql, ARRAY_N);
			if( is_array($vals) )
			{
				foreach( $vals as $v )
				{
	...
	public function inlineCss($style, $src, $fieldId)
	{
		...
		if( empty($css = $cached->pop()) )
		{
			...
			{
				$value = base64_decode(\get_option($fieldId, ''));
				if( !empty($value) )
				{
	 ...
	 * Or if the css path is followed by a stygen argument with stygen admin field id,
	 * we get or create a cache file and redirect url to it. */
	public function trackPseudoCss($src, $handle)
	{
		$part = explode('?', $src, 2);
		$args = array();
		parse_str( parse_url( $src, PHP_URL_QUERY ), $args );

		if( isset($args['stygen']) )
		{
			if( !empty(\get_option($args['stygen'])) )
			{
				$key = sanitize_key($args['stygen']);
				$cached = new Cache($key . '-cached.css');
				if( !$cached->isValid() )
				{
					$pseudocss = new self();
					$pseudocss->extract($this->relevantUrlPart($part[0]), false, false);
					$css = base64_decode(get_option($key, ''));
					if( !empty($css) )
					{
						$loading = new \LWS\Adminpanel\PseudoCss();
	...
	protected function getStandardFont()
	{
		...
		if( is_null($fonts) )
		{
			$path = LWS_ADMIN_PANEL_JS . '/resources/standard_fonts.json';
	...
	protected function fileIsValid($url)
	{
		...
		// ... must add more check if out of wordpress !!
...
//PseudoCss::tryGet(); // standalone usage
