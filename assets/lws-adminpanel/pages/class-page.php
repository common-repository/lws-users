...
class Page
{
	...
	public function registerMenu()
	{
		if( is_null($this->master) )
			$this->pageId = add_menu_page($this->title(), $this->title(), $this->data['rights'], $this->id, array($this, 'page'), isset($this->data['dashicons']) ? $this->data['dashicons'] : '', isset($this->data['index']) ? $this->data['index'] : null);
		else
	...
	function singularEnqueueScripts($hook)
	{
		if( $hook == $this->pageId && $this->isSingularEdit() )
		{
			...
			\wp_enqueue_script('lws-form-singularedit', LWS_ADMIN_PANEL_JS.'/singularedit.js', array('postbox'), LWS_ADMIN_PANEL_VERSION, true);
		}
	}
	...
	{
		if( isset($this->data['tabs']) && count(array_filter($this->data['tabs'], function($k){return $k!=\LWS\Adminpanel\Pages\Page::LIC_TAB_ID;}, ARRAY_FILTER_USE_KEY))>1 )
	...
	private function topBar()
	{
		...
		$copyright = sprintf(__('%1$s Admin Panel %2$s', LWS_ADMIN_PANEL_DOMAIN), 'LWS', LWS_ADMIN_PANEL_VERSION);
		...
		return "<div id='lws-TopBar'>$html</div>";
	}
	...
	protected function singularForm()
	{
		...
		echo "<div id='lws-adminpanel-singular-holder' class='metabox-holder columns-2'>"; // metabox holder

		echo "<div id='lws-adminpanel-singular-container-1' class='postbox-container lws-postbox-left'>"; // postbox-container 1
		echo "<div class='meta-box-sortables ui-sortable'>"; // meta-box-sortables
		...
		echo "<div id='lws-adminpanel-singular-container-2' class='postbox-container lws-postbox-right'>"; // postbox-container 2
		echo "<div id='lws-adminpanel-singular-actions' class='meta-box-sortables ui-sortable lws-adminpanel-singular-actions lws-adminpanel-singular-actionbox'>"; // meta-box-sortables
		if( $ok !== false )
		{
			$publish = '';
	...
	protected function getSingularPostbox($id, $title, $content, $css='')
	{
		...
		$class = 'lws-adminpanel-singular-metabox postbox';
		if( !empty($css) )
			$class .= ' ' . $css;
		$html = "<div$attrId class='$class'>";
		$html .= "<button type='button' class='handlediv' aria-expanded='true'><span class='screen-reader-text'>Toggle panel: {$title}</span><span class='toggle-indicator' aria-hidden='true'></span></button><h2 class='hndle ui-sortable-handle'><span>{$title}</span></h2>";
