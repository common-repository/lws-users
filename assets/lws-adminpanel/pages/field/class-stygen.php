...
class StyGen extends \LWS\Adminpanel\Pages\Field
{
	...
	public function input()
	{
		if($this->is_valid())
		{
			...
			{
				if( is_array($this->extra['subids']) )
				{
					foreach( $this->extra['subids'] as $sub )
					$lwsseditor .= "<input name='{$sub}' type='hidden' value='".\get_option($sub)."'/>";
				}
				else
				$lwsseditor .= "<input name='{$this->extra['subids']}' type='hidden' value='".\get_option($this->extra['subids'])."'/>";
			}
	...
	public function script()
	{
		...
		wp_enqueue_script( 'lws-stygen-font', LWS_ADMIN_PANEL_JS . '/stygen/stygenfont.js', array('jquery','js-cookie'), LWS_ADMIN_PANEL_VERSION, true );
		wp_enqueue_script( 'lws-stygen-fields', LWS_ADMIN_PANEL_JS . '/stygen/stygenfields.js', array('jquery'), LWS_ADMIN_PANEL_VERSION, true );
		wp_enqueue_script( 'lws-stygen-panel', LWS_ADMIN_PANEL_JS . '/stygen/stygenpanel.js', array('lws-stygen-fields','lws-stygen-font'), LWS_ADMIN_PANEL_VERSION, true );
		wp_enqueue_script( 'lws-stygen', LWS_ADMIN_PANEL_JS . '/stygen/stygen.js', array('lws-base64','lws-stygen-panel'), LWS_ADMIN_PANEL_VERSION, true );
