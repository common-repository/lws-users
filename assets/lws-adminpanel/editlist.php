...
class EditList
{
	...
	public function display()
	{
		...
		$this->filters($rcount, $limit, false);
		...
		\wp_register_script( 'lws-adminpanel-editlist', LWS_ADMIN_PANEL_JS.'/editlist.js', array('lws-tools', 'lws-base64', 'jquery', 'jquery-ui-dialog'), LWS_ADMIN_PANEL_VERSION, true);
		\wp_localize_script( 'lws-adminpanel-editlist', 'lws_editlist', array('confirmLeave' => __("Confirm or abort edition before continue.", LWS_ADMIN_PANEL_DOMAIN)) );
	...
	protected function addButton()
	{
		...
		{
			$ph = __("Add", LWS_ADMIN_PANEL_DOMAIN);
	...
	protected function editionForm($colspan=1)
	{
		...
		$ph = array(__('Cancel', LWS_ADMIN_PANEL_DOMAIN), __('Save', LWS_ADMIN_PANEL_DOMAIN));
	...
	protected function editButtons($id)
	{
		...
		$ph = apply_filters('lws_ap_editlist_item_action_names_' . $this->slug, array(
			self::DEL => __('Trash', LWS_ADMIN_PANEL_DOMAIN),
