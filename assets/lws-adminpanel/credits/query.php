...
class Query
{
	...
	private function authorisedUserTabContent($activated)
	{
		...
		if( $activated )
		{
			...
			$help = empty($num) ? __("You actually use a <b>hacked</b> version.", LWS_ADMIN_PANEL_DOMAIN) : sprintf(__("This activation actually use the license key <b>%s</b>", LWS_ADMIN_PANEL_DOMAIN), $num);
		...
		$groups['license'] = array(
			'title' => $activated ? __("Set another license key", LWS_ADMIN_PANEL_DOMAIN) : sprintf(__("%s - Activate your product", LWS_ADMIN_PANEL_DOMAIN), $data['Name']),
	...
	private function lambdaUserTabContent($activated)
	{
		...
		{
			return array(
				'title' => sprintf(__("%s - Activate your product", LWS_ADMIN_PANEL_DOMAIN), $data['Name']),
				'text' => sprintf(
