...
class ArgParser
{
	...
	{
		static $filtersAdded = false;
		if( !$filtersAdded )
		{
			$filtersAdded = true;
			\add_filter('lws_adminpanel_arg_parse', array($this, 'parse'));
			\add_filter('lws_adminpanel_post_parse', array($this, 'parsePost'));
			\add_filter('lws_adminpanel_arg_parse_opt', array($this, 'parseTransposed'), 10, 2);
			\add_filter('lws_adminpanel_post_parse_opt', array($this, 'parsePostTransposed'), 10, 2);
		}
	}
	...
	function parseTransposed($attrs, $options)
	{
		if( !is_array($options) )
			$options = array();
	...
	function parsePostTransposed($attrs, $options)
	{
		$attrs['post'] = true;
		$attrs['values'] = $_POST;
		return $this->parseTransposed($attrs, $options);
	}
	...
	function parse($args)
	{
		if( is_array($args) && isset($args['values']) && is_array($args['values']) )
		{
			...
			if( isset($args['post']) && boolval($args['post']) )
	 ...
	 * If the format value is an array, it assumes the repective value in values is an array too.
	...
	protected function formatValue(&$args, $key, $format, &$value)
	{
		...
		{
			if( !call_user_func($format, $value) )
