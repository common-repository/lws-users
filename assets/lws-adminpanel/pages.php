...
class Pages
{
	...
	*/
	...
	public static function currentPage()
	{
		...
		{
			if( isset($_GET['page']) )
				$lws_adminpanel_page = $_GET['page'];
			else if( isset($_POST['option_page']) )
				$lws_adminpanel_page = $_POST['option_page'];
	...
	public function hookCss($hook)
	{
		...
		{
			wp_enqueue_style('lws-adminpanel-wpcss', LWS_ADMIN_PANEL_CSS . '/wp.css', array(), LWS_ADMIN_PANEL_VERSION);
			...
			wp_enqueue_style('lws-adminpanel-colorselector', LWS_ADMIN_PANEL_CSS . '/colorselector.css', array('lws-adminpanel-css'), LWS_ADMIN_PANEL_VERSION);
	...
	public function enqueueScripts($scriptFilters=array())
	{
		...
		if( empty($todo) || isset($todo['lws-adminpanel-wpoverride']) )
			$this->enqueueScript( 'lws-adminpanel-wpoverride', '/wpoverride.js', array('jquery') );
...
/** @deprecated
		if( empty($todo) || isset($todo['lws-adminpanel-ace-editor']) )
			$this->enqueueScript( 'lws-adminpanel-ace-editor', '/ace/ace.js', array('jquery') );
*/
	...
	public function ajaxButton()
	{
		if( isset($_REQUEST['button']) && isset($_REQUEST['form']) )
		{
			...
			$form = $_REQUEST['form'];
			if( empty($button) || empty($form) )
				exit(0);
			$data = json_decode( base64_decode($form), true );
